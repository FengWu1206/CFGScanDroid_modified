...
public class CFGSig {
	...
	public CFGSig(ControlFlowGraph cfg) {
		...
		this.basicBlocks = cfg.getBasicBlocks();
		this.leaders = cfg.getLeaders();
		...
		Map addr_length = new HashMap();
		Map addr_operation = new HashMap();
		for (int i = 0; i<basicBlocks.size(); ++i){
			addr_length.put(leaders.indexOf(basicBlocks.get(i).startInstructionAddress), basicBlocks.get(i).instructionCount);
			for (int j=0; j<basicBlocks.get(i).instructions.size(); j++){
				if (basicBlocks.get(i).instructions.get(j).destinations != null){
					addr_operation.put(leaders.indexOf(basicBlocks.get(i).startInstructionAddress), basicBlocks.get(i).instructions.get(j).instruction.getOpcode());
				}
			}
		}
